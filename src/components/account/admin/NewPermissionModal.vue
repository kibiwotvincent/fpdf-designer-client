...
<template>
	<modal id="newPermissionModal" size="small">
		<Form @submit="onSubmit" :validation-schema="schema" v-slot="{ errors, isSubmitting }">
			<label class="block">Enter Permission Name</label>
...
<script>
	...
	export default {
		name: 'NewPermissionModalComponent',
		data: () => ({
			permission: {},
			successMessage: ''
		}),
		methods: {
			...
			async onSubmit(values, { setErrors }) {
				...
				return await http.post(process.env.VUE_APP_API_URL+'/api/admin/permissions/create', {'name': this.permission.name})
					.then((response) => {
